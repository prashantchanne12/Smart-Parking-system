...
public class signup extends AppCompatActivity{
    ...
    private TextInputEditText editText_full_name,editText_email_id,editText_password,editText_phone_number,editText_car_number;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editText_car_number = (TextInputEditText) findViewById(R.id.car_number);
    ...
    private void registerUser() {
        ...
        String car_number = editText_car_number.getText().toString().trim();
        ...
        if(car_number.isEmpty()){
            editText_car_number.setError("Please provide valid Car Number");
            editText_car_number.requestFocus();
            return;
        }
        ...
        mAuth.createUserWithEmailAndPassword(email_id,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    User user = new User(full_name,email_id,phone_number,car_number);

                    FirebaseDatabase.getInstance("https://smart-parking-system-1b2ec-default-rtdb.firebaseio.com/").getReference("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
